...
Bootstrapping 2 Kubernetes worker nodes. We already installed containerd and its dependencies on these nodes
...
### Prerequisites
The Certificates and Configuration are created on controlplane01 node and then copied over to workers using scp. Once this is done, the commands are to be run on first worker instance: node01. 
...
## Verification
Return to the controlplane01 node.
...
node01   NotReady   <none>   75s   v1.36.0
```
